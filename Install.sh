...
SwapSize='16'
KernelCMD='threadirqs intel_iommu=on iommu=pt split_lock_detect=off kvm.ignore_msrs=1 kvm.report_ignored_msrs=0 i915.enable_guc=3 i915.enable_fbc=0 nvidia-drm.modeset=1 nvidia.NVreg_UsePageAttributeTable=1 nvidia.NVreg_InitializeSystemMemoryAllocations=0 nvidia.NVreg_EnableStreamMemOPs=1 nvidia.NVreg_EnableResizableBar=1'
...
while getopts "w:f:u:p:h:r:l:t:s:c:k:b:m:a" opt
do
  case "$opt" in
    w ) WiFi="$OPTARG" ;;
    f ) WiFiPass="$OPTARG" ;;
...
BootSizeMiB="100"
SwapSizeMiB="$(expr $SwapSize \* 1024 + $BootSizeMiB)"
...
then
  ...
  then
    ...
    pacstrap -C cachy.conf /mnt --arch x86_64_v4 $Kernel ${Kernel}-headers
  else
    pacstrap -C chaotic.conf /mnt $Kernel ${Kernel}-headers
  fi
else
...
then
  ...
  Packages='kdenlive'
  Packages="$Packages qemu-desktop virt-manager dnsmasq edk2-ovmf swtpm"
...
pacstrap -P /mnt --needed libusb-compat usb_modeswitch smartmontools nvme-cli hdparm zsh grml-zsh-config thermald \
  ...
  networkmanager network-manager-applet networkmanager-openvpn wireguard-tools wireless-regdb \
  ...
  mesa lib32-mesa mesa-utils vulkan-icd-loader lib32-vulkan-icd-loader vulkan-tools ocl-icd lib32-ocl-icd libva-utils \
  ...
  neofetch htop galculator gparted ntfs-3g mediainfo-gui vlc obs-studio audacity fdkaac gimp steam firefox flatpak $Packages \
  xdg-user-dirs xdg-desktop-portal xdg-desktop-portal-gnome xdg-desktop-portal-gtk xdg-desktop-portal-xapp \
  yad gtk-engines gtk-engine-murrine spdlog lib32-dbus \
  openssl-1.1
pacstrap -U /mnt 'https://archive.archlinux.org/packages/t/transmission-gtk/transmission-gtk-3.00-6-x86_64.pkg.tar.zst'
...
[ -f chaotic.conf ] && yes '' | pacstrap -C chaotic.conf -i /mnt --needed ttf-ms-fonts qogir-gtk-theme-git qogir-icon-theme-git mangohud-git lib32-mangohud-git $Packages2
...
cat <<'XDG' > /usr/share/xdg-desktop-portal/portals.conf
[preferred]
default=xapp;gtk;
XDG
...
echo 'SUBSYSTEM=="block", ENV{ID_FS_TYPE}=="ntfs", ENV{ID_FS_TYPE}="ntfs3"' > /etc/udev/rules.d/10-ntfs3_by_default.rules
...
echo 'HandleLidSwitch=ignore' > /etc/systemd/logind.conf.d/notebook.conf
